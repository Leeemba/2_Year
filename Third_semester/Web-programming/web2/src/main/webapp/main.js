...
async function clearTable() {
    if (confirm("Вы уверены, что хотите очистить таблицу результатов?")) {
        const response = await fetch("controller?action=clear", { method: "GET" });
        if (response.ok) {
            // Очищаем таблицу на клиенте
            ...
            createNotification("Ошибка при очистке таблицы");
        }
    }
}
...
function drawPoint(x, y, r, result) {
    ...
    circle.style.fill = result ? "#00FF00" : "#FF0000";
...
function addToTable(x, y, r, result) {
    ...
    newRow.insertCell().innerHTML = result
        ? "<span class=\"success\">Есть пробитие!</span>"
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    try {
        const selectedRadio = document.querySelector("input[name=R-radio-group]:checked");
        if (selectedRadio) {
            r = selectedRadio.value;
        }
    } catch (err) {
        // Если ничего не выбрано, r остается undefined
    }
    ...
    if (table) {
        for (let item of table.rows) {
            ...
            const result = item.children[3].innerText.trim() === "Есть пробитие!";
    ...
    function click(element) {
        element.onclick = function () {
            ...
            this.style.boxShadow = "0 0 40px 5px #b39ddb";
            this.style.backgroundColor = "#b39ddb";
