...
function updatePointsData() {
    ...
    window.pointsData = [];
    const currentR = parseFloat(sessionStorage.getItem(STORAGE_KEYS.CURRENT_R)) || 1;
    ...
    rows.forEach(row => {
        ...
        if (cells.length === 6) {
            ...
            // Фильтровать точки по текущему R (с учетом погрешности)
            if (Math.abs(r - currentR) > 0.001) return;
...
function submitForm(actualX, actualY) {
    ...
    const formattedY = parseFloat(actualY).toFixed(7);

    if (window.displayPoint) {
        window.displayPoint.x = formattedX;
    }
...
function handleRChange(value) {
    ...
    if (window.updateCurrentR) {
        window.updateCurrentR(rValue);
    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const savedR = sessionStorage.getItem(STORAGE_KEYS.CURRENT_R) || 1;
    if (window.currentR !== undefined) {
        window.currentR = parseFloat(savedR);
    }
    setRValue(savedR);

    const xValue = sessionStorage.getItem(STORAGE_KEYS.X_VALUE) || 1;
    setXValue(xValue);

    const yValue = sessionStorage.getItem(STORAGE_KEYS.Y_VALUE) || 1;
    setYValue(yValue);
});